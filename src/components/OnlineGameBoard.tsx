...
interface GameState {
  ...
  roundScores?: Array<{ playerId: string; playerName: string; roundPoints: number; remainingTiles: number }> | null;
  currentRound?: number;
  totalRounds?: number;
...
export function OnlineGameBoard({ roomId, onBack, initialPlayers, initialGameState, playerName }: OnlineGameBoardProps) {
    ...
    const handleStateUpdate = ({ gameState: newGameState, players: newPlayers }: { gameState: any; players: any[] }) => {
      ...
      if (myPlayerIndex === newGameState.currentPlayerIndex) {
        if (newGameState.isNewLead && myPlayerName) {
          if (!lastNotifiedTurnRef.current) {
            lastNotifiedTurnRef.current = 'notified';
  ...
  return (
    <div className={styles.board}>
      ...
      {(gameState.phase === 'roundEnd' || gameState.phase === 'gameEnd') && gameState.roundScores && (
        <div className={styles.modalOverlay}>
          <div className={styles.modalCard}>
            ...
            <p className={styles.modalWinner}>
              {players.find(p => p.id === gameState.roundWinnerId)?.name}이(가) 먼저 모든 타일을 냈습니다!
            </p>
                  ...
                  .map((rs, i) => {
                    ...
                    return (
                      <tr key={rs.playerId}>
                        <td>{i + 1}</td>
                        <td>
                          {player?.name || rs.playerName} {rs.playerId === socket?.id && <span className={styles.youBadge}>나</span>}
                        </td>
              ...
              {gameState.phase === 'gameEnd' ? (
                ...
                <>
                  {socket?.id === gameState.hostId ? (
                    ...
                    <div className={styles.waitingText}>방장이 다음 라운드를 시작하기를 기다리는 중...</div>
                  )}
